...
# Runs all four prediction models, collects their results, and compares
# them against each other and the human baseline (ExpectedDurationMins).
# Human baseline: MAE=39.25, RMSE=57.26, R2=0.4785
...
def _print_comparison_table(all_results: list[dict]):
    ...
    # Identify best model (lowest MAE among ML models, excluding human baseline)
    ...
    # Compare best model to human baseline
...
# Creates grouped bar charts comparing MAE, RMSE, and R2 of each model
def _plot_metric_bars(all_results: list[dict]):
    ...
    fig.suptitle("Model Comparison Against Human Baseline", fontsize=15, fontweight="bold")
    ...
    plt.tight_layout()
    path = os.path.join(GRAPH_DIR, "model_comparison_bar_charts.png")
    plt.savefig(path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"  Saved bar chart comparison → {path}")


# Creates a 2x2 grid of predicted vs actual scatter plots, one per model
def _plot_scatter_grid(ml_results: list[dict]):
    fig, axes = plt.subplots(2, 2, figsize=(14, 12))
    fig.suptitle("Predicted vs Actual Duration – All Models", fontsize=15, fontweight="bold")
    axes = axes.flatten()
    ...
    plt.tight_layout()
    path = os.path.join(GRAPH_DIR, "model_comparison_scatter_grid.png")
    plt.savefig(path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"  Saved scatter grid        → {path}")


# Bar chart of cross-validation MAE scores with 95% CI error bars
def _plot_cv_comparison(ml_results: list[dict]):
    ...
    ax.set_title("5-Fold Cross-Validation MAE (95% CI)", fontsize=13, fontweight="bold")
    ...
    # Draw human baseline line
    ax.axhline(y=HUMAN_BASELINE["mae"], color="red", linestyle="--", linewidth=1.2,
               label=f"Human Baseline MAE ({HUMAN_BASELINE['mae']:.1f})")
    ax.legend()
    plt.tight_layout()
    path = os.path.join(GRAPH_DIR, "model_comparison_cv_mae.png")
    plt.savefig(path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"  Saved CV MAE comparison   → {path}")


# Runs every model, collects results, and produces comparison outputs.
# include_expected: whether to include ExpectedDurationMins as a feature
def run_all(include_expected: bool = True):
    variant = "with" if include_expected else "without"
    print(f"\n{'#' * 70}")
    print(f"#  RUNNING ALL MODELS ({variant} human estimate)")
    print(f"{'#' * 70}\n")
    ...
    results.append(lr_run(include_expected=include_expected))

    print("\n[2/4] Random Forest")
    results.append(rf_run(include_expected=include_expected))

    print("\n[3/4] Gradient Boosting (XGBoost)")
    results.append(gb_run(include_expected=include_expected))

    print("\n[4/4] Neural Network (MLP)")
    results.append(nn_run(include_expected=include_expected))

    # Combine with human baseline for comparison
    ...
    _plot_scatter_grid(results)      # ML models only
    _plot_cv_comparison(results)     # ML models only
...
if __name__ == "__main__":
    # Run with human estimate included
    run_all(include_expected=True)

    # Uncomment the line below to also run without the human estimate:
    # run_all(include_expected=False)
